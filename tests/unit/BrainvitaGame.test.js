...
describe('BrainvitaGame', () => {
    ...
    describe('Solution Playback', () => {
        test('should initialize solution player', () => {
            expect(game.currentSolution).toBe(null);
            expect(game.currentMoveIndex).toBe(0);
            expect(game.playbackInterval).toBe(null);
        });

        test('should handle solution selection', () => {
            const event = { target: { value: '0' } };
            game.handleSolutionSelect(event);
            expect(game.currentMoveIndex).toBe(0);
            expect(document.getElementById('playBtn').disabled).toBe(false);
        });

        test('should control playback speed', () => {
            const event = { target: { value: '5' } };
            game.updatePlaybackSpeed(event);
            expect(document.getElementById('speedRange').value).toBe('3');
        });
    });
